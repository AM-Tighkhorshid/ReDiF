...
def main(_):
    ...
    for epoch in range(config.num_epochs):
        ...
        # -------------------------------------------------
        # Shared noise (CRITICAL for fair comparison)
        # -------------------------------------------------
        batch_size = config.sample.batch_size
        latents_shape = (
            batch_size,
            student_pipeline.unet.config.in_channels,
            student_pipeline.unet.config.sample_size,
            student_pipeline.unet.config.sample_size,
        )

        init_latents = torch.randn(latents_shape, device=accelerator.device, dtype=prompt_embeds.dtype)
        ...
        with torch.no_grad():
            teacher_out = pipeline_with_logprob(
                teacher_pipeline,
                prompt_embeds=prompt_embeds,
                latents=init_latents.clone(),   # SAME NOISE
                num_inference_steps=config.sample.num_steps,
                guidance_scale=config.sample.guidance_scale,
                eta=config.sample.eta,
                output_type="pt",
        ...
        def to_tensor(imgs):
            if isinstance(imgs, list):
                imgs = torch.stack([T.ToTensor()(img) for img in imgs]).to(accelerator.device)
            return imgs

        teacher_images = to_tensor(teacher_images)
        student_images = to_tensor(student_images)

        # -------------------------------------------------
        # Optional: VAE latent space comparison (better signal)
        # -------------------------------------------------
        with torch.no_grad():
            teacher_latents = teacher_pipeline.vae.encode(teacher_images * 2 - 1).latent_dist.sample()

        student_latents = student_pipeline.vae.encode(student_images * 2 - 1).latent_dist.sample()

        # -------------------------------------------------
        # Losses
        # -------------------------------------------------
        pixel_loss = F.mse_loss(student_images, teacher_images)

        latent_loss = F.mse_loss(student_latents, teacher_latents)

        # weighted combination
        loss = pixel_loss + 0.5 * latent_loss
        ...
        print(f"Epoch {epoch}: Recon Loss = {total_loss_val:.6f}")
