...
def main(_):
    ...
    if kl_beta != 0:
        outdir = outdir + "_kl_" + str(kl_beta)
    stats_dir = outdir
    ...
    accelerator = Accelerator(
        ...
        # <--- CORRECTION: Accumulation is now handled once per trajectory, not per step
    ...
    # <--- CORRECTION: The reference pipeline for KL divergence should not have its weights updated
    # It represents the policy at the start of training (or the SFT model).
    # We can use the student_pipeline before training for this, or load a separate one.
    # For simplicity, we will use the student's state at the start of the epoch as the reference.
    # To do this correctly, we will use the `old_log_probs` from the sampling phase as our reference.
    ...
    if config.use_lora:
        student_pipeline.unet.requires_grad_(False)
        lora_attn_procs = {}
        ...
        unet = AttnProcsLayers(student_pipeline.unet.attn_processors)
    else:
    ...
    group_size = getattr(config.train, "group_size", 4)

    for epoch in range(config.num_epochs):
        logger.info(f"Epoch {epoch}: Sampling")

        prompts = [prompt_fn(**config.prompt_fn_kwargs)[0] for _ in range(config.sample.batch_size)]
        ...
        # Teacher sampling (only for reward calculation)
        with torch.no_grad():
        ...
        # Student sampling: This is the "old policy" (`π_θ_old`) from the paper
        ...
        for j in tqdm(range(config.student.num_steps)):
            # This is the `π_θ` (current policy) forward pass
        ...
        print(f"Epoch {epoch}: Loss = {total_loss_val:.4f}, Reward mean = {rewards.mean().item():.4f}, Reward std = {rewards.std().item():.4f}")
        ...
        if accelerator.is_main_process and epoch % 10 == 0:
            eval_prompts = [
                "A cat on a chair", "A boy in a forest", "A futuristic city skyline at night",
                "A dragon flying over mountains", "A cozy cabin in a snowy forest",
            ]
            
            with torch.no_grad():
                teacher_eval_images = [
                    teacher_pipeline(prompt, num_inference_steps=config.sample.num_steps, guidance_scale=config.sample.guidance_scale).images[0]
                    for prompt in eval_prompts
                ]
                student_eval_images = [
                    student_pipeline(prompt, num_inference_steps=config.student.num_steps, guidance_scale=config.sample.guidance_scale).images[0]
                    for prompt in eval_prompts
                ]
